...
class SessionBase(object):
    ...
    def close(self):
        ...
        if self._handle is None:
            warnings.warn(
                "Attempting to close NI-XNET session but session was already " "closed",
                errors.XnetResourceWarning,
            )
            return
    ...
    def can_comm(self):
        # type: () -> _utils.CanComm
        """:any:`nixnet._utils.CanComm`: CAN Communication state"""
        state_value_ctypes = _ctypedefs.u32()
        state_size = ctypes.sizeof(state_value_ctypes)
        _funcs.nx_read_state(
            self._handle,
    ...
    def application_protocol(self):
        ...
        return constants.AppProtocol(
            _props.get_session_application_protocol(self._handle)
        )
